...
public class SearchViewModel : ViewModelBase
{
    ...
    private void SelectData(object? obj)
    {
        ...
        {
            _context.Database.EnsureCreated();
            Books = new ObservableCollection<Book>(_context.Books.Where(x => x.ISBN == SecondCondition).ToList());
    ...
    private void EditItem(object? obj)
    {
        if (obj is not null)
        {
            ...
            else if (FirstCondition == "attended by Student with PESEL")
            {
                ...
                EditCourseViewModel editSubjectViewModel = new EditCourseViewModel(_context, _dialogService)
                {
    ...
    private void RemoveItem(object? obj)
    {
        if (obj is not null)
        {
            ...
            else if (FirstCondition == "attended by Student with PESEL")
            {
                ...
                Course? subject = _context.Courses.Find(subjectId);
                if (subject is null)
                {
                    return;
                }
                DialogResult = _dialogService.Show(subject.Title);
                if (DialogResult == false)
                {
                    return;
                }
